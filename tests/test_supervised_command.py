...
class SupervisedCommandTest(unittest.TestCase):
    ...
    def test_precontained_bwrap_flags_must_precede_the_command_separator(self):
        ...
        with self.assertRaisesRegex(
            SupervisedCommandError, "pre-contained command is invalid"
        ):
            run_supervised_command(
                [
                    bwrap,
    ...
    def test_helper_sigkill_cannot_orphan_detached_descendants(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            ready = root / "helper-kill-ready"
            late_mutation = root / "helper-kill-mutation"
            token = f"afk-helper-kill-{os.getpid()}-{time.monotonic_ns()}"
            child = (
                "import signal,time;"
