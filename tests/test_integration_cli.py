...
def write_project_contract(path: Path, *, repo_url: str = "https://github.com/acme/widgets") -> None:
    path.write_text(
        json.dumps(
            {
                "schema_version": 1,
...
""",
    ...
    def test_integrate_pr_keeps_pending_checks_non_actionable_when_follow_up_enabled(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            workstream_path = temp_path / "ledger" / "workstreams" / "run-pending-follow-up" / "workstream-result.json"
...
""",
            ...
            self.assertEqual(retrospective["integration_decision"], "checks_pending")
    ...
    def test_integrate_pr_creates_follow_up_for_exact_head_mismatch_when_enabled(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            workstream_dir = temp_path / "ledger" / "workstreams" / "run-follow-up"
            workstream_path = workstream_dir / "workstream-result.json"
            publication_path = workstream_dir / "publication-result.json"
            contracts_dir = temp_path / "contracts"
            beads_workspace = temp_path / "beads"
...
""",
            ...
            self.assertEqual(len(retrospective["follow_up"]["created"]), 1)
    ...
    def test_integrate_pr_uses_tracker_implementation_commit_when_current_repair_head_cannot_be_recovered(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            temp_path = Path(temp_dir)
            ledger = temp_path / "ledger"
            workstream_dir = ledger / "workstreams" / "run-repaired-head"
            workstream_path = workstream_dir / "workstream-result.json"
            publication_path = workstream_dir / "publication-result.json"
            first_result_path = ledger / "runs" / "implement-1" / "step-result.json"
            final_result_path = ledger / "runs" / "implement-2" / "step-result.json"
            ...
            first_result_path.write_text(
                json.dumps({"output": {"git": {"after_commit": "first-sha"}}}),
                encoding="utf-8",
            )
            ...
            workstream_path.write_text(
                json.dumps(
                    {
                        "workstream_id": "central-8xvj",
                        "tracker": {"implementation_commit": "final-sha"},
                        "publication": {
                            "status": "published",
                            "url": "https://github.com/acme/widgets/pull/17",
                        },
                        "steps": [
                            {
                                "name": "implement",
                                "result_path": "runs/implement-1/step-result.json",
                            },
                            {"name": "validate"},
                            {
                                "name": "implement",
            ...
            self.assertEqual(result["decision"], "merge_ready")
            self.assertEqual(result["expected_head_sha"], "final-sha")
            self.assertEqual(result["observed_head_sha"], "final-sha")
