...
def build_pi_real_worker_agent(
    *,
    ...
    if require_mounts and pi_coding_agent_dir is None and provider_name == "openai-codex":
        raise ValueError("agent.env.PI_CODING_AGENT_DIR is required")
...
def build_provider_pi_mount_config(
    *,
    ...
    provider_name = require_non_empty(provider, "provider")
    mount_config: dict[str, Any] = {"provider": provider_name}
...
def openai_codex_pi_mount_error(
    *,
    ...
    return f"{', '.join(missing)} {verb} required when {field_prefix}.provider is openai-codex"
...
def non_openai_pi_mount_error(
    *,
    ...
    return f"{', '.join(mounted)} {verb} only supported when {field_prefix}.provider is openai-codex"
